...
public class TKafkaInputStormcodeJava
{
  protected static String nl;
  public static synchronized TKafkaInputStormcodeJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TKafkaInputStormcodeJava result = new TKafkaInputStormcodeJava();
    nl = null;
    return result;
  }
...
for (IConnection conn : node.getOutgoingConnections()) {
    TupleWrapperHelper twhKafka = new TupleWrapperHelper(conn);
    // This is guaranteed to be hard-coded to "str".
    twhKafka.setCodeFieldName(conn.getMetadataTable().getListColumns().get(0), "\"str\"");
    twhKafka.generate();
} 
