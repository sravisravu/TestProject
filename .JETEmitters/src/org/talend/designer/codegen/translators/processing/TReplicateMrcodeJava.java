...
public class TReplicateMrcodeJava
{
  protected static String nl;
  public static synchronized TReplicateMrcodeJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TReplicateMrcodeJava result = new TReplicateMrcodeJava();
    nl = null;
    return result;
  }
...
if(outConnNames.isEmpty() || inConnName == null || metadata == null){
    return "";
}
class Mapper extends MapperHelper{
    public void map(){
        for(String key : outConnNames.keySet()){
            for(IMetadataColumn column : metadata.getListColumns()){
            
    stringBuffer.append(TEXT_119);
    stringBuffer.append(getOutValue(key));
