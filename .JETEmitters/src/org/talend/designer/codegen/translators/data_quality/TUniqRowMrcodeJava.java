...
public class TUniqRowMrcodeJava
{
  protected static String nl;
  public static synchronized TUniqRowMrcodeJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TUniqRowMrcodeJava result = new TUniqRowMrcodeJava();
    nl = null;
    return result;
  }
  ...
  protected final String TEXT_900 = " = value_";
  ...
  protected final String TEXT_916 = " = null;" + NL + "\t\t\t\t\t\t}else{" + NL + "\t\t\t\t\t\t\t";
  protected final String TEXT_917 = ".";
  protected final String TEXT_918 = " = ";
  protected final String TEXT_919 = ".";
  protected final String TEXT_920 = ".toLowerCase();" + NL + "\t\t\t\t\t\t}";
  protected final String TEXT_921 = NL + "\t\t\t\t\t";
  protected final String TEXT_922 = ".";
  protected final String TEXT_923 = " = ";
  protected final String TEXT_924 = ".";
  protected final String TEXT_925 = ";";
  protected final String TEXT_926 = NL + "\t\t\t\tboolean uniqueDone_";
  protected final String TEXT_927 = " = false;";
  protected final String TEXT_928 = NL + "\t\t\t\t\t";
  protected final String TEXT_929 = " ";
  protected final String TEXT_930 = " = key_";
  protected final String TEXT_931 = ";";
  protected final String TEXT_932 = NL + "\t\t\t\twhile(values_";
  protected final String TEXT_933 = ".hasNext()){" + NL + "\t\t\t\t\t";
  protected final String TEXT_934 = " ";
  protected final String TEXT_935 = " = values_";
  protected final String TEXT_936 = ".next();" + NL + "\t\t\t\t\tif(!uniqueDone_";
  ...
  protected final String TEXT_943 = NL + "\t\t\t\t\t\tuniqueDone_";
  protected final String TEXT_944 = " = true;" + NL + "\t\t\t\t\t\tcontinue;" + NL + "\t\t\t\t\t}" + NL;
  protected final String TEXT_945 = NL + "\t\t\t\t\t\tif(uniqueDone_";
  ...
  protected final String TEXT_952 = NL + "\t\t\t\t\t\t\t\t\tbreak;";
  protected final String TEXT_953 = NL + "\t\t\t\t\t\t}";
  protected final String TEXT_954 = NL + "\t\t\t\t}";
  protected final String TEXT_955 = NL + "\t\t\t\twhile(values_";
  protected final String TEXT_956 = ".hasNext()){" + NL + "\t\t\t\t\t\t";
  protected final String TEXT_957 = " ";
  protected final String TEXT_958 = " = values_";
  protected final String TEXT_959 = ".next();";
  ...
  public String generate(Object argument)
  {
	...
	final List<Map<String, String>> keyColumns = (List<Map<String,String>>)ElementParameterParser.getObjectValue(node, "__UNIQUE_KEY__");
	final List<? extends IConnection> connsDuplicate = node.getOutgoingConnections("DUPLICATE");
	final List<? extends IConnection> connsUnique = node.getOutgoingConnections("UNIQUE");
	String duplicateConnNameTemp = null;
	String uniqueConnNameTemp = null;
	if (connsDuplicate != null && connsDuplicate.size() > 0) {
      for (IConnection conn : connsDuplicate) {
        if (conn.isActivate()){
          duplicateConnNameTemp = conn.getName();
        }
      }
    }
    if (connsUnique != null && connsUnique.size() > 0) {
      for (IConnection conn : connsUnique) {
        if (conn.isActivate()){
          uniqueConnNameTemp = conn.getName();
        }
      }
    }
	final String duplicateConnName = duplicateConnNameTemp;
	final String uniqueConnName = uniqueConnNameTemp;
	final boolean hasDuplicateConnName = duplicateConnName!=null;
	final boolean hasUniqueConnName = uniqueConnName!=null;
	final boolean hasMultipleOutputs = hasDuplicateConnName && hasUniqueConnName;
	final String outConnName = uniqueConnName!=null ? uniqueConnName : duplicateConnName;
	...
	List<String> listUnqiueKeyTemp = new ArrayList<String>();
	List<IMetadataColumn> listUnqiueKeyMetadataTemp = new ArrayList<IMetadataColumn>();
	IMetadataTable inputMetadataTableTemp = null;
	...
	if(incomingConnections!=null){
		for(IConnection incomingConn : incomingConnections){
			if(incomingConn.getLineStyle().hasConnectionCategory(IConnectionCategory.DATA)){
				...
				inputMetadataTableTemp = incomingConn.getMetadataTable();
				for(IMetadataColumn inputCol : inputMetadataTableTemp.getListColumns()){
	...
	boolean hasKeyTemp = false;
	for(Map<String, String> keyColumn:keyColumns){
		if(("true").equals(keyColumn.get("KEY_ATTRIBUTE"))){
			if (inputMetadataTableTemp!=null) {
				for (IMetadataColumn column : inputMetadataTableTemp.getListColumns()) {
					if(keyColumn.get("SCHEMA_COLUMN").equals(column.getLabel())){
						listUnqiueKeyMetadataTemp.add(column);
						break;
					}
				}
			}
	...
	Map<String,String> columnIsCheckCaseSensitiveTemp = new HashMap<String,String>();
	for(Map<String, String> keyColumn:keyColumns){
		if(("true").equals(keyColumn.get("CASE_SENSITIVE"))){
			if (inputMetadataTableTemp!=null) {
				for (IMetadataColumn column : inputMetadataTableTemp.getListColumns()) {
					if(keyColumn.get("SCHEMA_COLUMN").equals(column.getLabel())){
						columnIsCheckCaseSensitiveTemp.put(column.getLabel(),"true");
						break;
					}
				}
			}
	...
	final List<IMetadataColumn> listUnqiueKeyMetadata = listUnqiueKeyMetadataTemp;
	final boolean hasKey = hasKeyTemp;
	final Map<String,String> columnIsCheckCaseSensitive = columnIsCheckCaseSensitiveTemp;


	final String combinerOutKeyClass = "mOutKey_"+cid;
	StructHelper keyStruct = new StructHelper(combinerOutKeyClass, listUnqiueKeyMetadata, true);
	keyStruct.generateCode();

	ComparatorHelper uniqRowKeyComparator = new ComparatorHelper();
	uniqRowKeyComparator.init("UniqRowKeyComparator_"+cid, keyStruct.columns, keyStruct.getStructName());
	uniqRowKeyComparator.generateCode();
	...
	class Mapper extends MapperHelper{ //---start for Mapper class
    ...
    stringBuffer.append(TEXT_906);
			...
			for(IMetadataColumn keyCol:listUnqiueKeyMetadata){//----11111-----
				JavaType javaType = JavaTypesManager.getJavaTypeFromId(keyCol.getTalendType());
				if(javaType == JavaTypesManager.STRING) {
					String caseSensitive = (String)columnIsCheckCaseSensitive.get(keyCol.getLabel());
					if(caseSensitive!=null && "true".equals(caseSensitive)){

    stringBuffer.append(TEXT_907);
    stringBuffer.append(getOutKey());
    stringBuffer.append(TEXT_908);
    stringBuffer.append(keyCol.getLabel());
    ...
    stringBuffer.append(TEXT_942);
							...
							if(isCombiner){
								this.output(getOutKey("unique"), getOutValue("unique"));
							}else{
								this.output(null, getOutValue("unique"));
							}
						}
    ...
    stringBuffer.append(getOutValue("duplicate"));
    ...
    stringBuffer.append(TEXT_951);
							...
							this.output(null, getOutValue("duplicate"));
	...
	} //---end for Reducer class

	Reducer reducer = new Reducer();
	if(hasMultipleOutputs){
		reducer.setType(R_TYPE_MO);
		Map<String, String> outConnNames = new HashMap<String, String>();
		outConnNames.put("unique", uniqueConnName);
		outConnNames.put("duplicate", duplicateConnName);
		reducer.init(cid, "mOutKey_"+cid, "mOutValue_"+cid, null, outConnNames);
	}else{
	...
	if(hasUniqueConnName && !hasDuplicateConnName){
