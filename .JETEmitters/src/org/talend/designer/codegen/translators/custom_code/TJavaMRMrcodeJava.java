...
public class TJavaMRMrcodeJava
{
  protected static String nl;
  public static synchronized TJavaMRMrcodeJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TJavaMRMrcodeJava result = new TJavaMRMrcodeJava();
    nl = null;
    return result;
  }
  ...
  public String generate(Object argument)
  {
	...
	for(IConnection incomingConn : node.getIncomingConnections()){
		if(incomingConn.getLineStyle().hasConnectionCategory(IConnectionCategory.DATA)){
			...
			inMetadataTable = incomingConn.getMetadataTable();
			break;
		}
	}
	...
	if(inMetadataTable == null || outMetadataTable == null)
		return "";

	boolean mapOnly = "true".equals(ElementParameterParser.getValue(node, "__MAP_ONLY__"));

	String mapCodeTemp = ElementParameterParser.getValue(node, "__MAP_CODE__");
	String reduceCodeTemp = !mapOnly ? ElementParameterParser.getValue(node, "__REDUCE_CODE__") : "";

	boolean mapPrep = "true".equals(ElementParameterParser.getValue(node, "__MAP_PREP__"));
	boolean mapConf = "true".equals(ElementParameterParser.getValue(node, "__MAP_CONF__"));
	boolean mapClose = "true".equals(ElementParameterParser.getValue(node, "__MAP_CLOSE__"));

	boolean reducePrep = "true".equals(ElementParameterParser.getValue(node, "__REDUCE_PREP__"));
	boolean reduceConf = "true".equals(ElementParameterParser.getValue(node, "__REDUCE_CONF__"));
	boolean reduceClose = "true".equals(ElementParameterParser.getValue(node, "__REDUCE_CLOSE__"));

	String mapPrepCodeTemp = mapPrep ? ElementParameterParser.getValue(node, "__MAP_PREP_CODE__") : "";
	String mapConfCodeTemp = mapConf ? ElementParameterParser.getValue(node, "__MAP_CONF_CODE__") : "";
	String mapCloseCodeTemp = mapClose ? ElementParameterParser.getValue(node, "__MAP_CLOSE_CODE__") : "";

	String reducePrepCodeTemp = (!mapOnly && reducePrep) ? ElementParameterParser.getValue(node, "__REDUCE_PREP_CODE__") : "";
	String reduceConfCodeTemp = (!mapOnly && reduceConf) ? ElementParameterParser.getValue(node, "__REDUCE_CONF_CODE__") : "";
	String reduceCloseCodeTemp = (!mapOnly && reduceClose) ? ElementParameterParser.getValue(node, "__REDUCE_CLOSE_CODE__") : "";

	//replace code
	Map<String, String> mrBaseMapper = new HashMap<String, String>();
	mrBaseMapper.put("key", "key_"+cid);
	mrBaseMapper.put("output", "output_"+cid);
	mrBaseMapper.put("reporter", "reporter_"+cid);
	mrBaseMapper.put("NULL", "NullWritable.get()");

	Map<String, String> mapInMapper = new HashMap<String, String>();
	mapInMapper.put("value", "value_"+cid);
	mapInMapper.put("InputKeyStruct", "NullWritable");
	mapInMapper.put("InputRowStruct", inConnName + "Struct");

	Map<String, String> outputKeyValueMapper = new HashMap<String, String>();
	outputKeyValueMapper.put("OutputKeyStruct", "NullWritalbe");
	outputKeyValueMapper.put("outputKey", "outputKey_"+cid);
	outputKeyValueMapper.put("OutputRowStruct", outConnName + "Struct");
	outputKeyValueMapper.put("outputRow", outConnName);

	Map<String, String> reduceInReducer = new HashMap<String, String>();
	reduceInReducer.put("values", "values_"+cid);

	Map<String, String> mrKeyValueClassMapper = new HashMap<String, String>();
	mrKeyValueClassMapper.put("mrKeyStruct", "mrKey_"+cid+"Struct");
	mrKeyValueClassMapper.put("mrValueStruct", "mrValue_"+cid+"Struct");

	Map<String, String> mrKeyValueMapper = new HashMap<String, String>();
	mrKeyValueMapper.put("mrKey", "mrKey_"+cid);
	mrKeyValueMapper.put("mrValue", "mrValue_"+cid);

	List<Map<String, String>> mapReplaceRules = new ArrayList<Map<String, String>>();
	mapReplaceRules.add(mrBaseMapper);
	mapReplaceRules.add(mapInMapper);
	if(mapOnly){
	mapReplaceRules.add(outputKeyValueMapper);
	}else{
	mapReplaceRules.add(mrKeyValueClassMapper);
	mapReplaceRules.add(mrKeyValueMapper);
	}

	List<Map<String, String>> reduceReplaceRules = new ArrayList<Map<String, String>>();
	reduceReplaceRules.add(mrBaseMapper);
	reduceReplaceRules.add(mrKeyValueClassMapper);
	reduceReplaceRules.add(reduceInReducer);
	reduceReplaceRules.add(outputKeyValueMapper);

	Map<String, String> mrConfBaseMapper = new HashMap<String, String>();
	mrConfBaseMapper.put("job", "job_"+cid);

	List<Map<String, String>> configureReplaceRules = new ArrayList<Map<String, String>>();
	configureReplaceRules.add(mrConfBaseMapper);

	class ReplaceUtil{
		public String rByRules(String original, List<Map<String, String>> rules){
			for(Map<String, String> rule : rules){
				for(String rKey : rule.keySet()){
					original = original.replaceAll("\\b"+rKey+"\\b", rule.get(rKey));
				}
			}
			return original;
		}
	}
	ReplaceUtil rUtil = new ReplaceUtil();

	final String mapCode = rUtil.rByRules(mapCodeTemp, mapReplaceRules);
	final String reduceCode = rUtil.rByRules(reduceCodeTemp, reduceReplaceRules);

	final String mapPrepCode = mapPrepCodeTemp;
	final String reducePrepCode = reducePrepCodeTemp;
	final String mapConfCode = rUtil.rByRules(mapConfCodeTemp, configureReplaceRules);
	final String reduceConfCode = rUtil.rByRules(reduceConfCodeTemp, configureReplaceRules);
	final String mapCloseCode = mapCloseCodeTemp;
	final String reduceCloseCode = reduceCloseCodeTemp;
	//replace code end
    ...
    stringBuffer.append(mapPrepCode);
    ...
    stringBuffer.append(mapConfCode);
    ...
    stringBuffer.append(mapCloseCode);
    stringBuffer.append(TEXT_906);
		...
		String mrKeyStructName = "mrKey_"+cid;
		String mrValueStructName = "mrValue_"+cid;

		List<Map<String, String>> keyTable = (List<Map<String,String>>)ElementParameterParser.getObjectValue(node, "__KEY__");
		List<IMetadataColumn> keyColumns = new ArrayList<IMetadataColumn>();
		MetadataColumn keyColumn;
		for(Map<String, String> keyRow : keyTable){
			keyColumn = new MetadataColumn();
			keyColumn.setLabel(keyRow.get("COLUMN_NAME"));
			keyColumn.setTalendType(keyRow.get("COLUMN_TYPE"));
			keyColumn.setNullable("true".equals(keyRow.get("COLUMN_NULLABLE")));
			keyColumns.add(keyColumn);
		}
		if(keyColumns.size() == 0){
			mrKeyStructName = null;
		}else{
			StructHelper mrKey = new StructHelper("mrKey_"+cid, keyColumns, true);
			mrKey.generateCode();
		}

		List<Map<String, String>> valueTable = (List<Map<String,String>>)ElementParameterParser.getObjectValue(node, "__VALUE__");
		List<IMetadataColumn> valueColumns = new ArrayList<IMetadataColumn>();
		MetadataColumn valueColumn;
		for(Map<String, String> valueRow : valueTable){
			valueColumn = new MetadataColumn();
			valueColumn.setLabel(valueRow.get("COLUMN_NAME"));
			valueColumn.setTalendType(valueRow.get("COLUMN_TYPE"));
			valueColumn.setNullable("true".equals(valueRow.get("COLUMN_NULLABLE")));
			valueColumns.add(valueColumn);
		}
		if(valueColumns.size() == 0){
			mrValueStructName = null;
		}else{
			StructHelper mrValue = new StructHelper("mrValue_"+cid, valueColumns, false);
			mrValue.generateCode();
		}
    ...
    stringBuffer.append(TEXT_914);
		...
		Mapper mapper = new Mapper();
		mapper.init(node, cid, null, inConnName, mrKeyStructName, mrValueStructName);
		mapper.generate();
    ...
    stringBuffer.append(reducePrepCode);
    ...
    stringBuffer.append(reduceConfCode);
    ...
    stringBuffer.append(reduceCode);
    ...
    stringBuffer.append(reduceCloseCode);
    stringBuffer.append(TEXT_922);
    
			}
		}
		Reducer reducer = new Reducer();
		reducer.init(cid, mrKeyStructName, mrValueStructName, null, outConnName);
		reducer.generate();
	}
