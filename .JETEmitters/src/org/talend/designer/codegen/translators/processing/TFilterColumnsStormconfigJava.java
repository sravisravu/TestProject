...
public class TFilterColumnsStormconfigJava
{
  protected static String nl;
  public static synchronized TFilterColumnsStormconfigJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TFilterColumnsStormconfigJava result = new TFilterColumnsStormconfigJava();
    nl = null;
    return result;
  }
  ...
  protected final String TEXT_46 = NL + "        stream_";
  protected final String TEXT_47 = " = ";
  protected final String TEXT_48 = NL + "        .project(";
...
// Get the metadata table for the filter flow
List<? extends IConnection> outFlowConns = node.getOutgoingConnections("FLOW");
IMetadataTable outFlowMetadataTable = null;
if(outFlowConns != null && outFlowConns.size() > 0 ){
    outFlowMetadataTable = outFlowConns.get(0).getMetadataTable();
}

if(outFlowMetadataTable == null) {
    return "";
}

List<Map<String, String>> tableValues = (List<Map<String,String>>)ElementParameterParser.getObjectValue(node, "__VALUES__");

String inStreamVariable = streamUtil.getCodeInStreamVariable();

// TODO: reject versus filter streams.
streamUtil.setOutConnection(outFlowMetadataTable != null ? outFlowConns.get(0) : null);

streamUtil.generateAllOutStreamsDeclarations();

    stringBuffer.append(TEXT_45);
    
    for (IConnection conn : node.getOutgoingConnections("FLOW")) { 
    stringBuffer.append(TEXT_46);
    stringBuffer.append( conn.getName() );
    stringBuffer.append(TEXT_47);
    stringBuffer.append( inStreamVariable );
    stringBuffer.append(TEXT_48);
    stringBuffer.append(new TupleWrapperHelper().getFields(conn));
