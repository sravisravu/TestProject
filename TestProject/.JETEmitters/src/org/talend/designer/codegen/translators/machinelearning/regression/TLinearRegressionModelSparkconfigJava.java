...
public class TLinearRegressionModelSparkconfigJava
{
  protected static String nl;
  public static synchronized TLinearRegressionModelSparkconfigJava create(String lineSeparator)
  {
    nl = lineSeparator;
    TLinearRegressionModelSparkconfigJava result = new TLinearRegressionModelSparkconfigJava();
    nl = null;
    return result;
  }

  public final String NL = nl == null ? (System.getProperties().getProperty("line.separator")) : nl;
  protected final String TEXT_1 = "org.apache.spark.ml.regression.LinearRegression lr_";
  protected final String TEXT_2 = " = new org.apache.spark.ml.regression.LinearRegression();";
...
// In linear regression, the label column must be of double format
boolean needsLabelIndexer = false;
String labelColumn = "indexedLabel";

// Linear regression parameters
String maxIter = ElementParameterParser.getValue(node,"__MAX_ITERATION__");// setMaxIter
String regularization = ElementParameterParser.getValue(node,"__REGULARIZATION__"); // setRegParam
String predictionColumn = "prediction"; // Unique name of the prediction column
String tolerance = ElementParameterParser.getValue(node,"__TOLERANCE__"); // setTol
String fitIntercept = ElementParameterParser.getValue(node,"__FIT_INTERCEPT__"); // setFitIntercept
String standardizeFeatures = ElementParameterParser.getValue(node,"__STANDARDIZE_FEATURES__");// setStandardization
String elasticNetParam = ElementParameterParser.getValue(node,"__ELASTICNET__");// setElasticNetParam
String solverAlgo = ElementParameterParser.getValue(node,"__SOLVER_ALGORITHM__");// setSolver

// Spark configuration
INode sparkConfigurationNode = null;
for (INode pNode1 : node.getProcess().getNodesOfType("tSparkConfiguration")) {
    if("tSparkConfiguration_1".equals(pNode1.getUniqueName())) {
        sparkConfigurationNode = pNode1;
        break;
    }
}

org.talend.hadoop.distribution.ESparkVersion sparkVersion = org.talend.hadoop.distribution.spark.SparkVersionUtil.getSparkVersion(sparkConfigurationNode);
org.talend.hadoop.distribution.ESparkVersion spark13Plus = org.talend.hadoop.distribution.ESparkVersion.SPARK_1_3;
org.talend.hadoop.distribution.ESparkVersion spark14Plus = org.talend.hadoop.distribution.ESparkVersion.SPARK_1_4;
org.talend.hadoop.distribution.ESparkVersion spark15Plus = org.talend.hadoop.distribution.ESparkVersion.SPARK_1_5;
org.talend.hadoop.distribution.ESparkVersion spark16Plus = org.talend.hadoop.distribution.ESparkVersion.SPARK_1_6;
...
Object[][] modelParameters = {
    {"setFeaturesCol", true, featuresColumn , "", spark13Plus},
    {"setLabelCol", true, needsLabelIndexer ? labelColumn : labelInputColumn,"// the transformed label column", spark13Plus},
    {"setPredictionCol", true, predictionColumn, "// predicted label (double)", spark13Plus},
    {"setMaxIter", false, maxIter, "// maximum number of iterations (int)", spark13Plus},
    {"setTol", false, tolerance, "// tolerance for stopping iteration (double)", spark14Plus},
    {"setElasticNetParam", false, elasticNetParam, "// elasticnet mixing parameter (double)", spark14Plus},
    {"setRegParam", false, regularization, "// regularization parameter (double)", spark13Plus},
    {"setFitIntercept", false, fitIntercept, "// whether to fit intercept term (boolean)", spark15Plus},
    {"setStandardization", false, standardizeFeatures, "// whether to standardize features before fitting the model (boolean)", spark15Plus},
    {"setSolver", true, solverAlgo, "// Sets the solver algorithm to use (string)", spark16Plus}
    };
...
for (Object[] modelParameter: modelParameters) {
    if (((org.talend.hadoop.distribution.ESparkVersion) modelParameter[4]).compareTo(sparkVersion) < 0) {
